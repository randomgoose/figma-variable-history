...
const { AutoLayout: AL, Text, h, Image } = figma.widget
...
function ArrowRight() {
    return <Text fontSize={11} width={16} height={16} fill={colors.text.secondary} horizontalAlignText="center" verticalAlignText="center">{'->'}</Text>
}

export async function generateChangeLog() {
    const commits: ICommit[] = getLocalCommits()

    const frames = await Promise.all(commits.map(async (commit: ICommit, index) => {
        const lastCommit = index <= commits.length - 1 ? commits[index + 1] : null

        const { added, modified, removed } = getVariableChanges({ current: commit.variables, prev: lastCommit ? lastCommit.variables : [] })
            ...
            <AL name={`Commit ${commit.id}`} width={960} padding={{ bottom: 48 }}>
                <AL direction="vertical" width={320} spacing={8}>
                    ...
                    <AL direction="vertical" spacing={4}>
                        ...
                        {/* Commit Description */}
                        <Text fontSize={11} fill={commit.description ? colors.text.default : colors.text.disabled}>{commit.description || 'No description'}</Text>
                    </AL>

                    {/* Collaborators */}
                    <AL verticalAlignItems="center" spacing={4}>
                        {commit.collaborators?.[0].photoUrl ? <Image width={18} height={18} src={commit.collaborators?.[0].photoUrl || ''} cornerRadius={100} /> : null}
                        <Text fontSize={11} fill={colors.text.secondary}>{commit.collaborators?.[0].name} on {parseDate(commit.date)}</Text>
                    </AL>
                </AL>
                ...
                <AL direction="vertical" width={'fill-parent'} spacing={16} name="Changes">
                    {
                        added?.length > 0
                            ? <AL direction="vertical" width={'fill-parent'} spacing={12}>
                                ...
                                <AL padding={16} wrap={true} width={'fill-parent'} fill={'#fcfcfc'} spacing={4} cornerRadius={6}>
                                    {added.map(v => (
                                        <AL key={v.id} width={149} height={20} verticalAlignItems="center">
                                            <Text fontSize={11} fontWeight={'normal'} truncate>{v.name}</Text>
                                        </AL>
                                    ))}
                                </AL>
                            </AL>
                                ...
                                <Text fontSize={11}>🔧 Changed</Text>
                                {
                                    modified.map(v => {
                                        ...
                                        return <AL direction="vertical" width={'fill-parent'} spacing={4} key={v.id}>
                                            ...
                                            <AL padding={16} width={'fill-parent'} fill={'#fcfcfc'} cornerRadius={6} direction="vertical" spacing={16}>
