...
export function getVariableChanges({ prev, current }: { prev?: Variable[], current: Variable[] }) {
    const added = prev ? current?.filter(v => !prev?.find(vc => vc.id === v.id)) : current ? current : [];
    const modified = prev ? current?.filter(v => prev?.find(vc => vc.id === v.id && Object.keys(diffVariables(v, vc)).length > 0)) : []
