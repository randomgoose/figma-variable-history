...
function CommitSelect({ commits, value, onChange, disabledAfter, showCurrent, isLoading }: ICommitSelectProps) {
  ...
  return <Select.Root
    ...
    onValueChange={(value) => {
      ...
      if (value !== 'current' && value !== '') {
        sendMessage(MESSAGE_TYPE.EXPORT_CODE, { commit: commits.find((commit) => commit.id === value) });
      }
    }}
  >
            ...
            isLoading ?
              ...
              : commits.length > 0 ?
                <>
                  ...
                  {commits.map((commit) => {
                    const disabled = (commit.date && disabledAfter) && (commit.date >= disabledAfter) ? true : false;
                    return <Select.Item disabled={disabled} key={commit.id} className={clsx("w-full dropdown-item text-white gap-1", {
...
export function DevModeApp() {
  ...
  useEffect(() => {
    ...
    if (currentCommit && prevCommit && currentCommit.date && prevCommit.date && (currentCommit.date <= prevCommit.date)) {
      setPrevCommitId('');
    }
  }, [currentCommitId])
  ...
  const groupedChanges = useMemo(() => {
    ...
    const currentCommit = currentCommitId === 'current' ? { variables, collections } : commits.find((c) => c.id === currentCommitId);
  ...
  const renderView = useCallback(() => {
    switch (view) {
      case 'variable':
        const numOfChanges = Object.values(groupedChanges).reduce((acc, { added, modified, removed }) => acc + added.length + modified.length + removed.length, 0);
        const hasUncommittedChanges = numOfChanges > 0;

        return numOfChanges > 0 ? <PanelGroup direction="vertical" className="h-full w-full">
          ...
          <PanelResizeHandle />
          {
            selectedVariableId && (
              <Panel className="border-y border-neutral-200">
                <VariableDetail
                  current={currentCommitId === 'current' ? variables.find((v) => v.id === selectedVariableId) : commits
                    .find((c) => c.id === currentCommitId)
                    ?.variables.find((v) => v.id === selectedVariableId)}
                  currentCollection={currentCommitId === 'current' ? collections.find((c) => c.id === selectedVariableId) : commits
                    ?.find((c) => c.id === currentCommitId)
                    ?.collections.find(
                      (c) =>
                        c.id ===
        ...
        </PanelGroup> : <EmptyState />;
      case 'css':
        return <Tabs.Root defaultValue="variables.css" className="h-full flex flex-col overflow-hidden border-b border-neutral-200">
    ...
    <div className="h-screen flex flex-col overflow-hidden">
      <div className="flex h-12 items-center gap-2 p-3 border-b border-neutral-200 sticky top-0 bg-[var(--figma-color-bg)] z-20">
        ...
        <CommitSelect
          commits={commits}
          value={prevCommitId}
          onChange={(value) => { setPrevCommitId(value); }}
          ...
          <Select.Portal>
            <Select.Content position="popper" className="dropdown-content w-32 z-50">
              ...
              <Select.Viewport className="w-full">
                {
                  views.map((view) => {
                    return <Select.Item key={view.key} value={view.key} className="dropdown-item text-white">
                      <Select.ItemText>
                        {view.label}
                      </Select.ItemText>
                    </Select.Item>
      ...
      <div className="flex items-center gap-2 p-3">
        <button className="btn-primary bg-blue-500" onClick={() => {
